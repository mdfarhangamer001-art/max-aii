...
export class MyraaAudioSession {
  ...
  private onUpdateSync?: (updateInfo: any) => void;
  ...
  constructor(handlers: {
    ...
    onUpdateSync?: (updateInfo: any) => void;
  }) {
    ...
    this.onUpdateSync = handlers.onUpdateSync;
  }
  ...
  public async connect() {
    ...
    try {
      ...
      const selectedVoice = localStorage.getItem("max_voice_id") || "voice_1";
      const selectedUserId = localStorage.getItem("max_user_id") || "";
      const wsUrl = `${protocol}//${window.location.host}/live?voice=${encodeURIComponent(selectedVoice)}${selectedUserId ? `&userId=${encodeURIComponent(selectedUserId)}` : ""}`;
      ...
      this.ws.onopen = async () => {
        ...
        try {
          ...
          // Start keepalive ping-pong interval to prevent network timeout
          ...
          }, 10000); // Ping every 10 seconds to maintain active channel
      ...
      this.ws.onmessage = async (event) => {
        try {
          ...
          // Handle update synchronization from Update Agent
          if (data.type === "update_sync" && data.updateInfo) {
            if (this.onUpdateSync) {
              this.onUpdateSync(data.updateInfo);
            }
          }
  ...
  // Helper to dispose nodes without resetting user's overall active state
  ...
  // Triggers automated low-latency reconnection logic on unintended closure
  private handleAutoReconnect() {
    ...
    setTimeout(async () => {
      // Guard against user manually toggling stop during wait delay
      if (!this.isActivated) {
        this.isAttemptingReconnect = false;
        return;
      }
